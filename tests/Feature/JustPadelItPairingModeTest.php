 ...
 *
 * Здесь — прежний режим записи парой (турниры без открытых пар). Турниры с
 * открытыми парами ходят через сетку, это JpiOpenPairsTest.
 *
...
class JustPadelItPairingModeTest extends TestCase
{
    ...
    private function makeTournament(string $pairingMode = 'admin', bool $paired = true): array
    {
        ...
        $t = Tournament::create([
            ...
            // Файл про прежний режим «сами игроки» — запись парой с партнёром.
            // Новая схема (открытые пары, сетка) проверяется в JpiOpenPairsTest.
            'open_pairs' => false,
