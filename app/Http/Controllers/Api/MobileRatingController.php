...
class MobileRatingController extends Controller
{
     ...
     * Место игрока в турнире.
     *
     * Расчёт переехал в App\Support\TournamentPlace: тем же местом теперь
     * пользуется лента амигос, и правило должно быть одно на всех.
     */
    private function getTournamentPlace($tournament, int $userId): ?int
    {
        return \App\Support\TournamentPlace::for($tournament, $userId);
    }
