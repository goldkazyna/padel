...
class MobileAuthController extends Controller
{
     ...
     * телефона). После подтверждения — АНОНИМИЗАЦИЯ: личные данные стираем, из
     * рейтинга убираем, телефон и соц-привязки освобождаем (можно
     * зарегистрироваться заново). Матчи НЕ удаляем — иначе сломается история
     * турниров у других игроков (матч общий на 4 человек).
     */
    public function deleteAccount(Request $request)
    {
        ...
        // Анонимизация: стираем личные данные, скрываем из рейтинга, освобождаем
        // телефон и соц-привязки (для повторной регистрации). Матчи сохраняем.
        $user->forceFill([
        ...
        // Убиваем все токены — войти в старый аккаунт больше нельзя.
        $user->tokens()->delete();
