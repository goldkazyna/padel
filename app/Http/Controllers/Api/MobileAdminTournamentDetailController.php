...
class MobileAdminTournamentDetailController extends Controller
{
    ...
    public function update(Request $request, Tournament $tournament): JsonResponse
    {
        ...
        // У парного JPI режим задаётся при создании и дальше не меняется:
        // смена обнуляет уже записавшихся — при «сами игроки» они хранятся
        ...
        if (!$typeChanged && ($request->has('has_playoff')
            ...
            if (!$hasPlayoff) {
                ...
                && empty($validated['playoff_type'])
                && empty($tournament->playoff_type)) {
                // См. создание: у Мексикано пустой тип уводит в полуфиналы,
                // которым нужно 8 игроков, и сетка молча не создаётся.
    ...
    private function formatDetail(Tournament $t, $user = null): array
    {
        ...
        return [
            ...
            'playoff_type' => $t->playoff_type,
            'playoff_format' => $t->playoff_format,
            'duration_hours' => $t->duration_hours !== null ? (int) $t->duration_hours : null,
            'is_rated' => (bool) $t->is_rated,
            'reserve_count' => (int) ($t->reserve_count ?? 0),
            'waitlist_size' => (int) ($t->waitlist_size ?? 0),
            'groups_count' => $t->groups_count !== null ? (int) $t->groups_count : null,
            'rounds_count' => $t->rounds_count !== null ? (int) $t->rounds_count : null,
            'teams_advance' => $t->teams_advance !== null ? (int) $t->teams_advance : null,
            'points_to_win' => $t->points_to_win !== null ? (int) $t->points_to_win : null,
    ...
    private function buildKingOfCourtLeaderboard(Tournament $tournament): array
    {
        ...
        $ranked = \App\Support\KingOfCourtRanking::standings($tournament);
    ...
    {
        // Единый порядок (см. JustPadelItScoring::sortKey): по очкам или по победам,
        // в зависимости от флага jpi_rank_by_wins.
        $byWins = (bool) $tournament->jpi_rank_by_wins;
        $players = $tournament->justPadelItPlayers
            ->sort(fn ($a, $b) => \App\Services\JustPadelItScoring::sortKey(
