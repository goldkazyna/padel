...
class MobileTournamentController extends Controller
{
    ...
    public function register(Request $request, Tournament $tournament)
    {
        ...
        if ($isWaitlisted) {
            ...
            $message = $friend
                ? "Вы и {$friend->name} в листе ожидания. Если место освободится, вас включат в основную сетку."
                : 'Вы в листе ожидания. Если место освободится, вас включат в основную сетку.';

            // В открытых парах очередь бывает двух разных видов, и просить
            // о действии можно только в одном из них.
            if ($tournament->usesOpenPairs()) {
                $freeSeats = \App\Support\OpenPairs::openTeams($tournament)->count();

                if ($freeSeats > 0 && !$friend) {
                    // Пары созданы все, но рядом с кем-то место пустует —
                    // человек может сесть сам, не дожидаясь никого.
                    $message = 'Вы в листе ожидания: все пары уже созданы. Но свободные места рядом с игроками ещё есть — нажмите «+» на любом из них и играйте, или попросите организатора подобрать вам пару.';
                } elseif ($freeSeats >= 2 && $friend) {
                    $message = "Вы и {$friend->name} в листе ожидания: все пары уже созданы. Свободные места рядом с игроками ещё есть — займите их кнопкой «+», или попросите организатора подобрать вам пары.";
                } else {
                    // Турнир занят целиком: просить не о чем, остаётся ждать.
                    $message = $friend
                        ? "Мест нет: все пары собраны. Вы и {$friend->name} в листе ожидания — если кто-то отменит запись, вы попадёте в основную сетку."
                        : 'Мест нет: все пары собраны. Вы в листе ожидания — если кто-то отменит запись, вы попадёте в основную сетку и сыграете.';
                }
            }
