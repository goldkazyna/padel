...
class TournamentController extends Controller
{
	...
	/**
	 * Завести пару: организатор добавляет сразу двоих.
	 *
	 * Куда именно ляжет пара — в пары формата или в команды турнира —
	 * решает PairRegistrationService, разница между режимами внутри него.
	 */
	public function addPair(Request $request, Tournament $tournament, \App\Services\PairRegistrationService $pairs)
	{
		$club = $this->getClub();
		if ($club && $tournament->club_id != $club->id) {
			abort(403);
		...
		[$ok, $message] = $pairs->addPair(
			$tournament,
			(int) $validated['player1_id'],
			(int) $validated['player2_id']
		);
	...
	public function seatPlayer(Tournament $tournament, int $userId, \App\Services\PairRegistrationService $pairs)
	{
		...
		[$ok, $message] = $pairs->seatPlayer($tournament, $userId);
	...
	public function sendPush(Request $request, Tournament $tournament)
	{
		...
		// выглядит как поломка рассылки. Ноль получателей объясняем — иначе
		// непонятно, номер не тот или на нём нет приложения.
		if (!empty($result['test_mode'])) {
			if ($sent === 0) {
				return back()->with('error',
					'ТЕСТОВЫЙ РЕЖИМ: ни один номер из PUSH_TEST_PHONES не получил push. '
					. 'Либо в базе нет пользователя с таким телефоном, либо у него нет '
					. 'устройства с установленным приложением (нужен вход в приложение '
					. 'хотя бы раз, чтобы записался push-токен).'
				);
			}
