   ...
   public function generatePlayoff(Tournament $tournament): bool
    ...
    protected function advanceWinner(TournamentPlayoffMatch $match): void
    {
        ...
        foreach ($all as $next) {
            ...
            // Продвижение всегда «вперёд»: победитель/проигравший не может попасть
            // в матч ТОЙ ЖE стадии. Иначе базовый source 'W'.match_number полуфинала
            // (напр. ПФ2 = 'W2') ложно совпадает со слотом другого полуфинала
            // ('W2' = победитель QF2) и затирает его.
            if ($next->stage === $match->stage) continue;
            $updated = false;
