...
class GroupSessionService
{
    ...
    protected function freezeCoachPay(ClubGroupSession $session, Club $club): void
    {
        ...
        // У группы может быть своя ставка «за клиента». Считаем по факту прихода:
        // сколько человек было на корте, включая пробных гостей. Посещаемость к
        // этому моменту уже записана. Если у группы цена за час, ставка тренера
        // тоже умножается на длительность — единица у них общая.
        $group = $session->group;
        $perClient = $group?->coachPriceForHours($session->hours());
        if ($perClient !== null) {
            $attended = ClubGroupAttendance::where('session_id', $session->id)
                ->where('attended', true)
                ->count();
            $booking->update(['coach_price' => $perClient * $attended]);
    ...
    /**
     * Сколько занятие приносит клубу: цена с человека × активные участники плюс
     * деньги пробных гостей. Если у группы цена за час, она умножается на
     * длительность занятия — иначе двухчасовое приносит столько же, сколько
     * часовое, а тренеру за него платится вдвое.
     *
    ...
    public function sessionRevenue(ClubGroupSession $session): float
    {
        ...
        return $group->priceForHours($session->hours()) * $members + $trial;
    }
