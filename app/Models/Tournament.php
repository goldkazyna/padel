...
class Tournament extends Model
{
     ...
     */
    /**
     * Личный турнир игрока — тот, у которого нет клуба.
     *
     * Раньше признаком был один `creator_id`, но его же проставляют этапам
     * лиги (кто из админов завёл вечер). Из-за этого клубный этап считался
     * личным и организатору не давали разослать пуш участникам.
     */
    public function isPersonal(): bool
    {
        return $this->creator_id !== null && $this->club_id === null;
    }
	 ...
	 *
	 * Клиенты присылают courts_count отдельно от названий, поэтому массив
	 * легко расходится со счётчиком: уменьшили корты — остались лишние
	 * названия, увеличили — не хватает. Приводим здесь, после каждого
	 * сохранения турнира. Счётчик может быть и не задан (Американо) — тогда
	 * длину не трогаем, только чистим пустые слоты.
	 */
	public function syncCourtNames(): void
	{
		...
		// Пустые слоты приводим к null и здесь: Американо счётчик кортов не
		// присылает (он считается от числа игроков в группе), поэтому без
		// этого в базу уходил бы мусор вида ["", "", "Центральный"], а
		// getCourtName() на пустой строке возвращал бы её вместо «Корт N».
