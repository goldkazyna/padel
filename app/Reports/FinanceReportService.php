...
class FinanceReportService
{
     ...
     * иначе — сотрудника, который её завёл в CRM. Онлайн-оплата (Plexy) всегда
     * за «Приложением»: деньги принимает шлюз, а не сотрудник.
     *
    ...
    public function paidBookings(Club $club, Carbon $from, Carbon $to): ReportSheet
    {
        ...
        foreach ($bookings as $b) {
            $entries[] = [
                ...
                'amount' => $this->bookingRevenue($b, $club->id),
                // Онлайн-оплата — канал приложения: деньги приходят через шлюз,
                // сотрудник их не принимает, даже если бронь завёл он.
                'seller' => $b->payment_method === 'plexy'
                    ? 'Приложение'
                    : ($sellers[(int) $b->id] ?? 'Не указан'),
                'row' => [
                    $this->parseDate($b->date)->format('d.m.Y'),
                    Carbon::parse($b->start_time)->format('H:i')
                        . '–' . Carbon::parse($b->end_time)->format('H:i'),
