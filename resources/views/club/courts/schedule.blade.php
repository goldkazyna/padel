                                ...
                                <td @if($span > 1) rowspan="{{ $span }}" style="padding: 4px;" @endif>
                                    ...
                                    <div class="slot {{ $slotClass }}{{ ($booking->club_card_id || $booking->source === 'app' || $booking->is_paid) ? ' has-icons' : '' }}"
                                         ...
                                         onclick="openViewModal({{ json_encode([
                                            ...
                                            'inventory' => $bookingInventory[$booking->id] ?? [],
...
<script>
    ...
    function openViewModal(data) {
        ...
        // Берём из данных самой брони — они не зависят от видимой даты, в
    ...
    function selectEditBookingType(btn) {
        ...
        // Инвентарь при group/tournament намеренно НЕ сбрасываем — блок сам
        // прячется через класс js-edit-hide-for-group (см. выше), а его
        // скрытые поля безвредны: контроллер для этих типов зовёт sync()
        // с пустым набором сам, присланный inventory не читает. Если сброс
        // выбора всё же вернуть сюда, клик «обычный → групповая → обратно
        // обычная» в одном открытом окне стирал бы валидный набор навсегда —
        // без сброса он просто снова становится видимым при возврате.
    }
