                                ...
                                <div class="left">
                                    ...
                                    @if($b->coach_id || $b->comment)
                                        <span class="meta">
                                            ...
                                            @php $coachPaidView = $b->booking_type === 'group' ? true : $b->coach_paid; @endphp
                                            @if($coachPaidView !== null)<span class="ws-coach-paid {{ $coachPaidView ? 'paid' : 'unpaid' }}" title="{{ $coachPaidView ? 'Тренер оплачен' : 'Тренер не оплачен' }}"></span>@endif</span>{{ $b->coach->first_name }}</span>@endif
...
<script>
    ...
    function openViewModal(data) {
        ...
        if (editCoachPriceGroupW && editCoachPriceW) {
            if (data.coachId) {
                editCoachPriceGroupW.style.display = '';
                if (data.coachPrice !== null && data.coachPrice !== undefined) {
                    editCoachPriceW.value = Math.round(data.coachPrice);
                } else {
                    ...
                    editCoachPriceW.value = rate > 0 ? Math.round(rate * hours) : '';
                }
            } else {
    ...
    function selectBookCoach(btn) {
        ...
        if (isActive) {
            ...
            btn.classList.add('active'); input.value = coachId;
