{{-- resources/views/club/tournaments/justpadelit/partials/_rounds.blade.php --}}
<div>
    <h6 class="text-white mb-3"><i class="bi bi-layers-fill text-primary me-2"></i>Раунды</h6>
    <div class="rounds-grid">
        @foreach($tournament->justPadelItRounds as $round)
            @php
            ...
            <div class="round-card {{ $statusClass }}" data-round-id="{{ $round->id }}">
                <div class="round-header" onclick="toggleJpiRound('jpi-round-{{ $round->id }}')" style="cursor: pointer;">
                    ...
                    <div class="round-header-right">
                        ...
                        <i class="bi bi-chevron-down collapse-icon {{ $isActive ? '' : 'collapsed' }}" id="icon-jpi-round-{{ $round->id }}"></i>
                    </div>
                </div>
                <div class="round-matches collapsible-content {{ $isActive ? '' : 'collapsed' }}" id="jpi-round-{{ $round->id }}">
                    ...
                    @foreach($round->matches as $match)
                                ...
                                <div class="match-vs">
                                    @if($match->isCompleted() && $tournament->status !== 'completed')
                                        <button class="btn-score-edit"
                                                ...
                                                data-bs-target="#jpiEditScoreModal{{ $match->id }}"
                                    ...
                                    @elseif(!$match->isCompleted() && $round->isInProgress())
                                        <button class="btn-score"
                                                data-bs-toggle="modal"
                                                data-bs-target="#jpiScoreModal{{ $match->id }}">
                        ...
                        @if($round->isInProgress() && !$match->isCompleted())
                            <div class="modal fade" id="jpiScoreModal{{ $match->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content modal-dark">
                                        ...
                                        <form action="{{ route('club.justpadelit.saveScore', $match) }}" method="POST">
                                            @csrf
                        ...
                        @if($match->isCompleted())
                            <div class="modal fade" id="jpiEditScoreModal{{ $match->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content modal-dark">
                                        <div class="modal-header border-0">
                                        ...
                                        <form action="{{ route('club.justpadelit.updateScore', $match) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal-body">
                                                <div class="score-input-grid">
    ...
    @if($tournament->status === 'in_progress')
        @php
            ...
            $currentRoundNumber = $tournament->justPadelItRounds->max('round_number') ?? 0;
        @endphp

        @if($canGenerateNext)
            <div class="text-center mt-4">
                <form action="{{ route('club.justpadelit.nextRound', $tournament) }}" method="POST"
