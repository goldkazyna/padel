...
@endif
{{-- Редактор групп (Американо, до начала турнира) --}}
@if($tournament->isAmericano() && $tournament->status === 'open')
    @include('club.tournaments.partials._groups_editor')
@endif
{{-- Американо --}}
@if($tournament->isAmericano() && $tournament->groups->count() > 0 && $tournament->status !== 'open')
