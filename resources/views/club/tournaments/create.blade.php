...
<div class="card-body">
				...
				@endif
                    ...
                    <div class="mb-4">
                        ...
                        @php $venueClubSelected = null; @endphp
                        @include('club.tournaments.partials._venue_club_picker')
                    </div>
					...
					<div id="escaleraFields" style="display: none;">
						<div class="alert-info-custom mb-4">
							...
							играются три коротких матча — каждый с каждым в паре. Двое первых на корте поднимаются
							выше, двое последних опускаются ниже. В таблицу идут не очки, а позиция игрока
						...
						<div class="mb-3">
							...
							<small class="text-secondary mt-2 d-block">
								...
								очки плюс бонус за результат матча — победа +2, ничья +1, поражение 0.
							</small>
						</div>
					</div>
					...
					<div id="americanoFlexFields" style="display: none;">
						<div class="alert-info-custom mb-4">
							...
							когда «Следующий раунд» и когда «Завершить турнир». Минимум игроков — количество_кортов × 4; чтобы кто-то отдыхал и очередь работала, нужен хотя бы один сверх этого.
						</div>
						<div class="row">
							<div class="col-md-6 mb-4">
								<label class="form-label">Количество кортов *</label>
								<input type="number" name="flex_courts_count" id="flexCourtsCount" class="form-control"
									   ...
									   oninput="generateCourtsInputs()">
								<small class="text-secondary">Сколько кортов реально играет. Игроков нужно минимум кортов × 4 (2 корта → 8); от девяти начинается ротация отдыха, ради которой формат и нужен.</small>
							</div>
						</div>
						<div class="mb-3">
							<div class="form-check">
...
// Поиск клуба-площадки живёт в partials/_venue_club_picker.blade.php
</script>
<style>
