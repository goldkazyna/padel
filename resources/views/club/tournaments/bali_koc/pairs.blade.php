...
<div class="page-header">
    <div>
        <h2>Создать пары</h2>
        <p>{{ $tournament->name }} · Король Корта (Bali Format)</p>
    </div>
...
@else
    ...
    <div class="card-dark">
        <div class="card-body">
            <form action="{{ route('club.bali-koc.storePairs', $tournament) }}" method="POST" id="pairsForm">
                @csrf
    ...
    <script>
    // Игроки, отсортированные по рейтингу для авто-распределения
    const players = @json($participants->map(fn($p) => ['id' => $p->id, 'rating' => (int) $p->rating])->values());

    // Запрет дубликатов: при выборе игрока в одном селекте — выключаем
    // его опцию в остальных селектах.
