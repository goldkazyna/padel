...
- **Внимание: Task 1 и Task 2 уже выполнены, и по ходу их ревью формат уточнился.** Ранжирование внутри корта теперь всегда по сумме очков, без выбора: два режима оказались математически тождественны (доказано перебором 4,8 млн комбинаций). Выбор перенесён в итоговую таблицу — поле `escalera_standings_mode` со значениями `points` (сумма баллов за позиции, по умолчанию) и `raw_points` (сумма очков за все матчи). Фрагменты кода в Task 1 и Task 2 отражают состояние до этого уточнения — ориентируйтесь на реальный код в репозитории и на спеку, а не на них.
- Позиции и баллы считаются **всегда**, независимо от режима таблицы: от них зависят перемещения между кортами и награда «Восхождение». Режим влияет только на сортировку итоговой таблицы.
- Сумма очков игрока за все матчи копится в `escalera_players.total_raw_points`.
...
- Produces:
  - `EscaleraService::rankCourt(EscaleraRoundCourt $court): array` — id игроков в порядке мест с первого по четвёртое
...
- Produces:
  ...
  - `standings(Tournament $tournament): array` — сортировка по режиму `escalera_standings_mode`
...
В `resources/views/club/tournaments/create.blade.php` добавить блок эскалеры по образцу блока Just Padel It: количество кортов (от 2 до 10), очков в коротком матче (8, 10, 12, 16 — по умолчанию 12), режим итоговой таблицы. Блок показывается только при выборе типа «Эскалера».

Режим итоговой таблицы — поле `escalera_standings_mode`, два значения:

- `points` (по умолчанию) — «По баллам за позиции»;
- `raw_points` — «По сумме очков за матчи».

Под переключателем короткое пояснение на русском: по баллам считается родной зачёт формата, где выгодно подниматься на корт выше; по сумме очков в зачёт идут все набранные очки, и тогда игроку становится выгоднее оставаться внизу и обыгрывать более слабых.

Выбора режима ранжирования **внутри корта** в форме нет: там всегда сумма очков.
